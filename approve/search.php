...
mysql_connect('localhost', 'www', 'www') or die("Error connecting to database: ".mysql_error());     
 ...
 // you will be able to search pretty much anything that is in the database.  Pretty clever!
  ...
  $sql = "SELECT id, fname, lname, email, phone, cfname, clname, cemail, cphone, rcnum, subtsk, title, region, cart, total, mfname, mlname, caar, afname, alname, approve FROM quotes WHERE caar IS NOT NULL ORDER BY id DESC";
 echo '<table class="from_db" cellpadding="6" cellspacing="0">';
		...
		echo '<tr><td width=250>Funding Manager</td><td width=550>'. $results["mfname"]. " " . $results["mlname"]. '</td></tr>';
		echo "<tr><td width=250>CAAR</td><td>". $results["caar"]."</td></tr>";
		echo '<tr><td width=250>Approving Manager</td><td width=550>'. $results["afname"]. " " . $results["alname"]. '</td></tr>';
		echo '<tr><td width=250>Approved</td><td>'; 
		if ($results["approve"] == 1) {
...
if ($counter % 2 == 0){
		...
		$total = $results["total"];
		$cart = $results["cart"];
		$mfname = $results["mfname"];
		$mlname = $results["mlname"];
		$caar = $results["caar"];
		$afname = $results["afname"];
		$alname = $results["alname"];
		$approve = $results["approve"];
		
		echo '<tr><td colspan="2">
		<form action="approve.php" method="POST" name="edit" />
		<input type="hidden" value="'. $rcnum .'" name="rcnum" />
		<input type="hidden" value="'. $afname .'" name="afname" />
		<input type="hidden" value="'. $alname .'" name="alname" />
		<input type="hidden" value="'. $caar.'" name="caar" />';
		if ($caar == TRUE && $approve == FALSE) {
		echo '<center><input type="submit" value="Approve Quote" class="button"></center></form></td></tr>'; 
		}
		elseif ($caar == FALSE && $approve == FALSE){
		echo '<center><font color="red">Please enter CAAR</font> </center></form></td></tr>'; 
		}
		else {
		echo '<center><font color="lime">Already approved!</font></center></form></td></tr>';
		}
