...
mysql_connect('localhost', 'www', 'www') or die("Error connecting to database: ".mysql_error());     
 ...
 // you will be able to search pretty much anything that is in the database.  Pretty clever!
  ...
  $sql = "SELECT id, fname, lname, email, phone, cfname, clname, cemail, cphone, rcnum, subtsk, title, region, cart, total, mfname, mlname, caar FROM quotes ORDER BY id DESC";
 echo '<table class="from_db" cellpadding="6" cellspacing="0">';
		...
		echo '<tr><td width=250>Approving Manager</td><td width=550>'. $results["mfname"]. " " . $results["mlname"]. '</td></tr>';
...
if ($counter % 2 == 0){
		...
		echo '<tr><td colspan="2">
		<form action="caar.php" method="POST" name="edit" />
		...
		<center><input type="submit" value="Enter CAAR" class="button" ></center></form></td></tr>';
